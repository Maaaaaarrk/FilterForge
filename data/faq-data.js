    ...
    },
    {
      ...
      "questions": [
        ...
        },
        {
          ...
          "answer": "<p>This is a critical distinction that trips up many filter authors:</p><ul><li><code>ED</code> &mdash; Reads the item's <strong>inherent</strong> Enhanced Defense (on armor) or Enhanced Damage (on weapons). Does <strong>NOT</strong> include bonuses added by runewords, socketed gems, runes, or jewels.</li><li><code>EDEF</code> &mdash; Reads the <strong>total</strong> Enhanced Defense including all bonuses from runewords and sockets.</li><li><code>EDAM</code> &mdash; Reads the <strong>total</strong> Enhanced Damage including all bonuses from runewords and sockets.</li></ul><p>Use <code>ED</code> when checking base item quality (e.g., superior items with 15% ED). Use <code>EDEF</code>/<code>EDAM</code> when you want the full picture including socketed/runeword bonuses.</p><pre><code>// Find superior items with 15% enhanced defense\nItemDisplay[SUP ARMOR ED&gt;14]: %NAME% %GREEN%[%ED%%]\n\n// Show total enhanced damage on runeword weapons\nItemDisplay[RW WEAPON]: %NAME%{ED: %EDAM%%}</code></pre>",
